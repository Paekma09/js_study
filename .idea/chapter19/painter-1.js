...
function createCanvas(canvasWidth, canvasHeight) {
    ...
    //그리기 도구를 mousedown 이벤트의 이벤트 리스너를 등록한다
    canvas.addEventListener("mousedown", function (e) {
        //FireFox 대책 : 색상을 선택하면, chage 이벤트를 강제로 발생시킨다
...
paintTools.brush = function (e, ctx) {
    ...
    //canavs 요소에 대한, 마우스 포인터의 상대 위치를 구한다
...
paintTools.line = function (e, ctx) {
    ...
    var img = ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height);
    //canvas요소에 대한 마우스 포인터의 상대 위치를 구한다
    var p = relativePosition(e, ctx.canvas);
...
//색상 선택 (선과 채우기를 모두 설정함 -> 필요하다면 두 개의 기능을 분리해서 별도의 컨트롤로 만드세요)
controls.color = function (ctx) {
    var input = colorInput = elt("input", { type: "color" });
    var label = elt("label", null, " 색 : ", input);
    input.addEventListener("change", function (e) {    //참고 : FireFox에서는 chage 이벤트가 발생하지 않습니다
...
//필터 툴
var filterTools = Object.create(null);  //필터툴 객체
...
filterTools.blur = function (inputImage) {
    ...
    var worker = new Worker("weightedAverageFilter.js");
    worker.postMessage({
...
filterTools.emboss = function (inputImage) {
    ...
    var worker = new Worker("weightedAverageFilter.js");
    worker.postMessage({
        image: inputImage, n: 1, Weight: W, keepBrightness: false, offset: 120
    });
    return worker;
};
...
//url을 ctx에 그린다(그림이 Canvas 경계선에 내접하도록 한다)
