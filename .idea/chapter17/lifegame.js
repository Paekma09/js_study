...
window.onload = function () {
    readFile("./patterns.json", function (jsonObj, error) {
        ...
        //bodu 요소 안에 생명 게임의 각 부품(controls, view)을 생성해서 배치한다
...
state.create = function (nx, ny) {
    ...
    //clickview 이벤트 리스너 등록 : view가 발생시킨 이벤트에 반응하여 셀의 상태를 바꾼다
    ...
    state.changeGenrationEvent = document.createEvent("HTMLEvents");
...
state.tellGenerationChange = function (generation) {
    state.changeGenrationEvent.initEvent("changegeneration", false, false);
    state.changeGenrationEvent.detail = { generation: generation };
    document.dispatchEvent(state.changeGenrationEvent);
};
...
state.update = function () {
    ...
    for (var ix=0; ix<state.nx; ix++) {
        for (var iy=0; iy<state.ny; iy++) {
            ...
            if ( sum <= 1 || sum >= 4 ) {   //주위의 마리 수가 한 마리 이하이거나 네 마리 이상이면 죽는다
                if ( state.cells[ix][iy] ) {
                    changedCell.push({ x: ix, y: iy });
                    //셀의 변경을 요청한다
                    state.tellCellChange(ix, iy, 0);
                }
            } else if ( sum == 3 ) {    //주위의 마리 수가 세 마리면 생성한다
                if ( !state.cells[ix][iy] ) {
                    changedCell.push({ x: ix, y: iy });
                    //셀의 상태 변경을 요청한다
                    state.tellCellChange(ix, iy, 0);
    ...
    //전체 셀의 상태를 확인하고 셀의 상태를 변경한다(배타적 논리합의 결과 0->1, 1->0이 된다)
...
view.create = function (nx, ny, width, height) {
    ...
    view.layer = [];
    //생물을 표시하는 레이어
    view.layer[0] = elt("canvas", { id: "layer0", width: width, height: height });
    //격자 선을 표시하는 레이어
    view.layer[1] = elt("canvas", { id: "layer1", width: width, height: height });
    //격자 크기, 셀의 크기, 생물을 표시하는 원의 반지름을 설정한다
    view.nx = nx;
    view.ny = ny;
    ...
    //렌더링 매개 변수의 초기 설정
    view.backColor = "forestgreen"; //배경 색상 설정
    view.markColor = "white";       //생물 샛강
    view.strokeStyle = "black";     //격자 선의 색상
    view.lineWidth = 0.2;           //격자 선의 굵기
    //격자를 그린다
    view.drawLattice();
    //세대를 표시하는 요소를 생성한다
...
controls.pattern = function (state) {
    ...
    function placePattern(pattern) {
        ...
        for (var i=0; i<array.length; i++) {
            for (var j=0; j<2; j++) {
                if ( array[i][j] > max[j] ) max[j] = array[i][j];
                if ( array[i][j] < min[j] ) min[j] = array[i][j];
            }
        }
        //모든 셀을 지운다
        state.clearAllCell();
